...
with open("./sim_inequality_dataout.txt", "a") as output_summary, open("./sim_inequality_full_dataout.txt", "a") as output_specific:
    ...
    #for i in (6,7,8):
    if True:
        for i in range(int(reps/(reps//4))):
            for i in range(3,10):
                #start = clock()
                out, inequalitiesAttractorGamesAllOutcomes, inequalitiesAttractorGamesNashOutcomes = runGameFairness(i, reps//4, mode="attractors", parallelize=True)
                summarywriter.writerow( out )
                print( out )
                out, inequalitiesAllGamesAllOutcomes, inequalitiesAllGamesNashOutcomes = runGameFairness(i, reps//4, mode="all_games", parallelize=True)
                summarywriter.writerow( out )
                print( out )
                ### prep raw gini scores, for error bars
                
                if True:
                    #giniwriter.writerow((["attractor_gamess", "all_outcomes", i, reps//4, ] + list(inequalitiesAttractorGamesAllOutcomes)))
                    giniwriter.writerow((["attractor_gamess", "nash_outcomes", i, reps//4, ] + list(inequalitiesAttractorGamesNashOutcomes)))
                    #giniwriter.writerow((["all_games", "all_outcomes", i, reps//4, ] + list(inequalitiesAllGamesAllOutcomes)))
                    giniwriter.writerow((["all_games", "nash_outcomes", i, reps//4, ] + list(inequalitiesAllGamesNashOutcomes)))
